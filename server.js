...
server.get("/getList/searchContent/:s",function(req,res){
...
server.get("/getAuthorList/authorName/:aname",function(req,res){
    console.log(req.params.aname);
    var configFile = fs.readFileSync("./authorData.json");
   var jsObj = JSON.parse(configFile);
   var searchedList = [];
   for (x in jsObj)
   {
       if(jsObj[x].authorName == req.params.aname)
       {
           searchedList.push(jsObj[x]);
       }    
   }
...
server.get("/editAuthor/authorName/:aname",function(req,res){
   console.log(req.params.bi);
    var configFile = fs.readFileSync("./authorData.json");
    var jsObj = JSON.parse(configFile);
    var len = jsObj.length;
    ...
    for(i in jsObj)
    {
        if(jsObj[i].authorName == req.params.aname)
        {
...
server.post("/saveAuthor",function(req,res){
    ...
    {   
        ...
        for(var i in jsObj)
        {
            ...
            {
                console.log("The Author to be edited is found"+jsObj[i].bookId+" "+req.body.bookId);
                jsObj[i] = req.body;
